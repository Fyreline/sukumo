...
1. Layout per ARCHITECTURE §1 (empty modules with docstrings where later phases own
   ...
   `node_modules/`, `dist/`.
2. Server: app factory, config (`HONMARU_` prefix), db.py, `security.py`/`identity.py`/
   `routers/auth.py` ported from Michi (delta: `role` assignment, AUTH §1),
   `routers/health.py`. Dev db `data/honmaru.dev.db`.
3. Web: Vite + React 19 + TS + Tailwind v4 + motion; port `auth.ts` (key
   `honmaru_auth`), `api.ts`, LoginScreen, ThemeToggle, empty BridgePage behind the
   auth gate. Dev port **5179**.
4. Theme: add `DST_HONMARU` to `learningLanguageMachine/scripts/sync-theme.sh`, update
   the canonical header's MIRRORS list, run it, commit the mirrored `theme.css` here.
5. PWA shell: manifest + placeholder keep icon + service worker registering and
   precaching the shell (network-first `/api` comes in Phase 4 with real data).
6. `.claude/launch.json`: `honmaru-api` (8301), `honmaru-web` (5179).
