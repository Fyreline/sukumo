...
| Thing | Value |
|---|---|
| API prod | LaunchAgent `com.honmaru.api`, uvicorn `127.0.0.1:8300`, `data/honmaru.db`, no `--reload` |
| API dev | launch.json `honmaru-api`, port `8301`, dev db |
| Web dev | launch.json `honmaru-web`, port `5179` |
| Web prod | GitHub Pages from `apps/web` build (Michi's workflow, `VITE_API_BASE=https://honmaru-api.mishka-hub.com`) |
| Tunnel | `honmaru-api.mishka-hub.com` → `http://127.0.0.1:8300` |
| Coach | `com.honmaru.coach`, StartInterval 900 |
| Journal | `com.honmaru.journal`, daily 02:30 |
| Backup | `com.honmaru.backup`, daily 03:30 |
...
```yaml
  - hostname: honmaru-api.mishka-hub.com
    service: http://127.0.0.1:8300
```

Then `cloudflared tunnel route dns <tunnel-name> honmaru-api.mishka-hub.com`, restart
...
`~/Library/Logs/honmaru/`. **Standing household lessons (learned on Kakeibo):**
...
a live db) → `data/backups/honmaru-<ts>.db`, prune to 30. The DB *is* the private data
...
Health Auto Export automation (API.md §2 config), ntfy app subscribed to the private
topic, Shortcuts automations (office geofence ×2, reading one-tap), PWA installed from
the Pages URL. Each item has a verify step in Phase 8 — a deploy isn't done until a
