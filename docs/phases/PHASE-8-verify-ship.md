...
## Build
1. LaunchAgents per DEPLOYMENT §1/3: `com.honmaru.api|coach|journal|backup` —
   `.venv/bin/python` direct (the TCC lesson), logs under `~/Library/Logs/honmaru/`.
...
5. Phone setup per DEPLOYMENT §5: HAE automation → prod URL, ntfy subscription,
   office geofence + reading Shortcuts, PWA install from Pages.
...
- [ ] HAE hourly automation hits prod (sync_runs shows it); kill it → 36h later the
      ops rule surfaces in the briefing (or time-travel test the rule against prod db
      copy).
...
- [ ] PLAN §6 success-criteria review scheduled: calendar occasion "Honmaru six-week
